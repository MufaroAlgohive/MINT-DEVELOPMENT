    ...
    if (registry.items?.length) {
      ...
      registry.items = activeItems
        ...
        .map(item => {
          ...
          console.log(`[PRICE-DEBUG public/token] item isin=${item.isin} | sec.id=${sec?.id} | sec.last_price=${sec?.last_price} | intradayRow=${JSON.stringify(intradayBySecId[sec?.id])} | livePrice=${livePrice} | FINAL cents=${livePriceCents} | FINAL rands=${(livePriceCents/100).toFixed(2)}`);
          return {
            ...item,
