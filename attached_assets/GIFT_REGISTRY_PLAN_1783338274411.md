...
**Status:** Phase 1–3 scaffolding is BUILT (tables, endpoints, pages, atomic reservation, sweeper cron). Phase 4 and several money/compliance pieces are **NOT** built yet — see "What We Don't Have Yet" below. Do not treat this as production-ready until that section is cleared.
...
The app already has **direct gifting** (send one share to one person by email + OTP). The Gift Registry is a separate feature — a shareable, live wishlist of shares and investments that multiple people can fund over time. The core skeleton (create a wishlist, add items, publish, share a link, reserve a share, "pay" for it) has been built. The parts that make it a real, safe, money-moving feature are still missing — see the gap analysis immediately below.

---

## ⚠️ What We Don't Have Yet (Gap Analysis vs. the Spec) — added 2026-07-06, before any further building

Explained simply, like the registry is a vending machine:

1. **The "pay" button doesn't really charge anyone's card yet.** Right now when a gifter clicks pay, the app just writes "PAID" in the database. No money actually leaves the gifter's bank/card and none of that money goes anywhere real. Paystack/Ozow is not wired in at all for this feature.
2. **The shares are never actually bought.** Even after something is marked "PAID," no order is sent to the stock exchange. The `order_ref` and `executed_amount_cents` fields exist in the table but are never filled in — a contribution can never become "SETTLED," only "PAID." So today, a gift registry contribution doesn't put real shares in anyone's account.
3. **Gifting to someone without a MINT account (the "OTHER, not signed up yet" case) isn't connected to anything.** The spec says this should behave exactly like today's direct-gift claim flow (hold it, email them a claim link, they sign up and KYC, then they get it). None of that plumbing — escrow, claim email, `gift_claims` linkage — exists for registry gifts yet.
4. **Gifting to a child isn't really wired to the child's account.** "CHILD" is just a label you can pick when creating a registry — the app doesn't check that the child has a real custodial account, and doesn't verify the guardian is KYC-complete before letting them create the registry.
5. **Share splits (corporate actions) aren't handled at all.** The decision was made (pause the item and ask the creator to fix the quantity) but nothing in the code watches for a split and pauses anything.
6. **Nobody gets told anything.** When someone gifts an item, the person who made the wishlist should get a notification ("Sipho gifted you 5 MTN shares!"). None of the new notification types (item funded, registry completed, expiry reminder, registry expired, gift received, contribution receipt email) have been built — only the plumbing for older, unrelated notifications exists.
7. **Refunds aren't built for this feature.** If a registry is cancelled or expires with money already collected, there is no refund path — the spec says to reuse the existing gift refund logic, but that reuse hasn't happened.
8. **No editing after creation.** The spec expects a creator can edit title/occasion/expiry/message after publishing (`PUT /:id`) — that endpoint doesn't exist; once published, a registry's details can't be changed (only paused/resumed/cancelled).
9. **The public link's privacy rules haven't been double-checked against the strict "never expose personal data" allowlist** the spec calls for — worth a dedicated pass before this is public-facing.
10. **No automated tests / acceptance-criteria checks** from Section 21 of the spec have been run against the build.

**Bottom line in one sentence:** the wishlist "looks and clicks" like the real thing end-to-end, but underneath, no real money moves and no real shares get delivered yet — items 1–3 above are the ones that matter most before this goes anywhere near real users.
...
4. **Show who gifted each item — confirmed: full name AND email, shown publicly on the wishlist.** ⚠️ This overrides the spec's Section 9.2 privacy rule and the project's threat model (Information Disclosure category), which both call for first-name-only on public pages. **RESOLVED 2026-07-06:** flagged directly to the user, who explicitly confirmed to proceed anyway and accept the privacy trade-off ("Show full name + email publicly, I accept the privacy risk"). This is a deliberate, informed product decision, not an oversight — keep this note here so future engineers know it was a conscious call, not a bug.

**Build order for this change (confirmed, ready to build once user gives the final go-ahead):**
...
- Add gifter full name + email display on both the owner's detail page and the public wishlist page, per the confirmed decision above.
